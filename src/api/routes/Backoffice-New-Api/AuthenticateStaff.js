...
router.post("/", function(request, response){
    ...
    try{
        ...
        req.execute("dbo.Authenticate_Staff", function(err, data){
            if(err){
                ...
                req2.input('action',sql.NVarChar(10), action);
                req2.input('actiondate',sql.NVarChar(500), actiondate);
                req2.input('staffid',sql.Int, data.recordset[0].fld_staffid);


                req2.execute("dbo.Authenticate_Credentials_Staff", function(err2, data2){
                    if(err2){
